...
int _open_file(db_t* db, char *filename, size_t init_size) {
...
uint64_t db_query(db_t *db, db_query_t query) {
  ...
  if (has_addresses) {
    ...
    for (size_t j = 0; j < query.addresses.len; ++j) {
      addresses[j] = murmur64A(query.addresses.data[j], sizeof(db_address_t), HASH_SEED);
    }
  }
  ...
  for (int i = 0; i < TOPICS_LENGTH; ++i) {
    ...
    for (size_t j = 0; j < query.topics[i].len; ++j) {
      has_topics = true;
      topics[i][j] = murmur64A(query.topics[i].data[j], sizeof(db_hash_t), HASH_SEED);
    }
  }
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (db->logs_capacity == db->logs_count) {
      ...
      if (
        ...
        ftruncate(db->topics_fd, db->logs_capacity * sizeof(db_hash_t) * TOPICS_LENGTH) != 0
      ) {
        errno = ERR_FAILED_REALLOC_FILE;
        return;
      }
